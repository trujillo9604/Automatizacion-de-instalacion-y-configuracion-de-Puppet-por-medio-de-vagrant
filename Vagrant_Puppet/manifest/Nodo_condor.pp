...
file { "/home/ubuntu/sources.list":
    ensure => 'present',
    owner  => 'root',
    group  => 'root',
    mode   => '644',
    source => 'puppet:///modules/condor/sources.list',
}

#Sobreescritura del archivo source.list

 exec { "CP_SOURCE": 
    command => '/bin/cp -rp /home/ubuntu/sources.list /etc/apt/sources.list',
}
...
file { "/home/ubuntu/condor_config.local":
    ...
    group  => 'root',
    mode   => '644',
    source => 'puppet:///modules/condor/condor_config_nodo.local',
}
